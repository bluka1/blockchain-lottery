...
const featuresData = [
	{
		...
		description:
			'Powered by Chainlink VRF. Every winner is chosen through provably fair, verifiable on-chain entropy.',
	},
	{
		icon: '⚡',
		title: 'Instant Payouts',
		description:
			'Winners are paid directly to their wallet within minutes of the draw.',
	},
];

export function HomePage() {
	const { wallet, connectWallet, isCorrectChain, switchToExpectedChain } =
		useWeb3Context();
	...
	const phaseText =
		wallet && !isCorrectChain
			...
			: lotteryData.phase !== null
				...
				: 'loading...';
	const roundText =
		lotteryData.currentRound !== null ? `#${lotteryData.currentRound}` : '#--';
		...
		<article className='home-page'>
			...
			{lotteryData.jackpot && (
				<div className='jackpot-display'>
					...
					<span className='jackpot-amount'>
						{parseFloat(lotteryData.jackpot).toFixed(4)} ETH
					</span>
				</div>
			)}
			...
			{!wallet ? (
				...
				<div className='network-warning'>
					<p>
						⚠️ Wrong network. Switch MetaMask to{' '}
						<strong>{CONTRACT_CONFIG.chainName}</strong> to play.
					</p>
				...
				<div className='participate-section'>
					...
					{lotteryData.userEntry?.exists ? (
						...
						<div className='info-message'>
							🎲 This round is closed and the draw is in progress. A new round
							will open shortly — come back in a moment to pick your numbers.
						</div>
					) : (
						<>
							...
							{txState.error && (
								<div className='error-message'>❌ {txState.error}</div>
							)}
							...
							{showSuccess && !txState.loading && (
								<div className='success-message'>
									...
									{txState.txHash && (
										<span className='tx-hash'>
											{' '}
											TX: {txState.txHash.substring(0, 10)}…
										</span>
									)}
									<br />
									The draw runs automatically when the countdown below hits
									zero.
								</div>
							)}
						</>
			...
			{wallet && isCorrectChain && (
				<section className='draw-status'>
					{lotteryData.phase === Phase.Open && lotteryData.nextDrawTime ? (
						<>
							<TimerToNextDraw
								targetTimestamp={lotteryData.nextDrawTime}
								offsetSeconds={chainTimeOffset}
							/>
							<p className='draw-hint'>
								When the countdown reaches zero, the draw is triggered
								automatically (Chainlink Automation + VRF) — you don't need to
								do anything. If you win, claim your prize from the Withdraw box
								above.
							</p>
						</>
					) : lotteryData.phase === null ? (
						...
						<p className='draw-hint'>
							🎲 Draw in progress — results are being finalized on-chain. Hang
							tight, this only takes a moment.
						</p>
					)}
				</section>
			)}
